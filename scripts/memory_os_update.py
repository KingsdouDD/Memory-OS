    ...
    for p in personas:
        ...
        try:
            ...
            with driver.session() as session:
                session.run(
                    """
                    MERGE (p:Persona {summary: $summary})
    ...
    try:
        ...
        for cand in candidates:
            ...
            if layer == "L0":
                ...
                summary = (cand.get("summary") or "")[:120]
                if summary:
                    from process_dream import _delete_by_filter
                    res = _delete_by_filter(summary)
                    deleted["l1"] += res.get("qdrant_deleted", 0)
    finally:
...
def confirm_update_4layer(token, selected_pids=None, new_memory=None):
    ...
    candidates = data.get("candidates", [])
    # selected_pids 过滤（支持 pid 或 summary 字符串）
    if selected_pids:
        filtered = []
        for c in candidates:
            pid_str = str(c.get("pid")) if c.get("pid") is not None else ""
            summary = c.get("summary", "")[:80]
            for sp in selected_pids:
                sp_str = str(sp)
                if pid_str == sp_str or summary == sp_str:
                    filtered.append(c)
                    break
        candidates = filtered
    if not candidates:
        return {"error": "没有可更新的目标"}
    # L1 优先（L3 不直接更新），同层按分数
    layer_order = {"L1": 0, "L2": 1, "L0": 2, "L3": 3}
    target = sorted(candidates, key=lambda c: (layer_order.get(c.get("layer", "L3"), 3), -c.get("score", 0)))[0]
    ...
    try:
        if target_layer == "L1":
            ...
            if l1_kos:
                from process_dream import _execute_update_v5, _find_pid_by_summary
                client = _qdrant_client()
                report = {"update": 0, "qdrant_written": 0, "qdrant_updated": 0, "neo4j": {"entities": 0, "relations": 0}}
                # 优先用 target 里的 pid + collection（快捷模式）
                real_pid = target_pid if target_pid else None
                real_coll = target_collection if target_collection else None
                if not real_pid:
                    # fallback: 从 summary 反查
                    summary = target.get("summary", "")[:120]
                    real_pid, real_coll = _find_pid_by_summary(summary)
                if not real_pid:
                    updated["l1"] = {"error": f"无法定位 pid（summary={target.get('summary','')[:50]}）"}
                else:
                    _execute_update_v5(
                        l1_kos[0], l1_kos[0].get("memory_type", "fact"),
                        real_coll, client, real_pid, report
                    )
                    updated["l1"] = {**report, "real_pid": str(real_pid), "real_collection": real_coll}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target-pid", default=None, help="直接指定要更新的 PID（跳过召回）")
    ...
    if args.command == "ingest":
        ...
        if not args.query:
            print(json.dumps({"error":"update 需要 --query"}))
            sys.exit(1)
        from recall_4layer import recall_4layer
        ...
        if not candidates:
            print(json.dumps({"phase":"confirm","action":"update","candidates":[],
                              "message":"未召回到候选记忆"}))
            sys.exit(0)
        # 快捷模式：传了 target_pid + target_collection + target_layer + file，直接调 confirm_update_4layer
        if args.target_pid and args.target_collection and args.target_layer and args.file:
            ...
            _save_action_token(fake_token, {
                "action":"update","query":args.query,"candidates":[{
                    "pid": args.target_pid, "layer": args.target_layer,
            ...
            sys.exit(0)
        token = _gen_action_token()
        _save_action_token(token, {"action":"update","query":args.query,"candidates":candidates})
        ...
        if args.file:
            with open(args.file, encoding="utf-8") as f:
                new_memory = json.load(f)
        print(json.dumps(confirm_action(args.token, selected_pids=selected, new_memory=new_memory),
                         ensure_ascii=False, indent=2))
